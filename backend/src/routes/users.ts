/**
 * Staff (system users) management — admin only.
...
router.get('/directory', authenticate, (_req: Request, res: Response) => {
  res.json({
    users: db.prepare(`
...
router.get('/', (_req: Request, res: Response) => {
  ...
  const rows = db.prepare(
    `SELECT ${publicCols} FROM users ${includeInactive ? '' : 'WHERE active = 1'} ORDER BY full_name ASC`
  ).all();
  res.json({ users: rows });
});
...
router.post('/', (req: Request, res: Response) => {
  ...
  try {
    db.prepare(`
      INSERT INTO users (id, email, password_hash, full_name, role, cpf, council_number, council_state)
      VALUES (?,?,?,?,?,?,?,?)
    `).run(id, d.email.toLowerCase(), bcrypt.hashSync(d.password, 10), d.full_name, d.role,
...
// Deactivate when the user has history (appointments, encounters, journal…),
// hard delete only when they never touched the system. Cannot remove yourself
// or the last active admin.
router.delete('/:id', (req: Request, res: Response) => {
  const target = db.prepare(`SELECT id, email, full_name, role FROM users WHERE id = ?`).get(req.params.id) as any;
  if (!target) { res.status(404).json({ error: 'not_found' }); return; }
  if (target.id === req.user!.id) { res.status(409).json({ error: 'cannot_delete_self' }); return; }
  if (target.role === 'admin') {
    const admins = (db.prepare(`SELECT COUNT(*) AS c FROM users WHERE role = 'admin' AND active = 1 AND id != ?`).get(target.id) as any).c;
    if (admins === 0) { res.status(409).json({ error: 'last_admin' }); return; }
  }
  const refs = (db.prepare(`
    SELECT (SELECT COUNT(*) FROM appointments WHERE practitioner_id = ?) +
           (SELECT COUNT(*) FROM encounters WHERE practitioner_id = ?) +
           (SELECT COUNT(*) FROM prescriptions WHERE practitioner_id = ?) +
           (SELECT COUNT(*) FROM journal_entries WHERE created_by = ?) AS c
  `).get(target.id, target.id, target.id, target.id) as any).c;
  if (refs > 0) {
    db.prepare(`UPDATE users SET active = 0, updated_at = ? WHERE id = ?`).run(new Date().toISOString(), target.id);
    logAudit({
      actorId: req.user!.id, actorEmail: req.user!.email,
