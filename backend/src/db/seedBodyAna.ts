/**
 * Seed Ana Beatriz Lima with a full 4-view clinical body capture set
 * (front/left/right/back) — overweight baseline for scenario image-gen testing.
 */
...
const SEED_DIR = path.join(__dirname, '../data/seed-body-ana');
...
export function seedAnaBodyCaptures(
  ...
  if (missing.length) {
    console.warn(`  ⚠ Ana body seed images missing: ${missing.join(', ')} (${SEED_DIR})`);
    return null;
  }
  ...
  `).run(
    ...
    opts.patientId,
    now,
    JSON.stringify({ seeded: true, views: VIEWS, patient: 'ana_beatriz_lima' }),
    opts.createdBy,
    now,
    now,
  );
  ...
  // Younger adult overweight baseline (Ana ~1985)
  db.prepare(`
    INSERT INTO body_measurements
      (id, tenant_id, patient_id, height_cm, weight_kg, waist_cm, notes, recorded_at, recorded_by, bmi, payload)
    VALUES (?, ?, ?, 165, 98.0, 112, ?, ?, ?, 36.0, ?)
  `).run(
    uuid(),
    opts.tenantId,
    opts.patientId,
    'Seed Ana — composição corporal (teste multi-vista, sobrepeso)',
    now,
    opts.createdBy,
    JSON.stringify({ hip_cm: 122, body_fat_pct: 44.0, seeded: true }),
  );
  ...
  `).run(
    ...
    opts.tenantId,
    opts.patientId,
    'Déficit calórico Ana',
    'Plano nutricional seed — proteína elevada com déficit controlado',
    'Proteína elevada, déficit ~500 kcal',
    JSON.stringify({ daily_calories: 1700, deficit_kcal: 500, protein_g: 120 }),
  );
  db.prepare(`
    INSERT INTO body_lifestyle_plans
      (id, tenant_id, patient_id, title, description, weeks, status, plan_type, summary, params_json)
  ...
  `).run(
    ...
    opts.patientId,
    'Força + caminhada',
    'Plano de treino seed — 3x resistência, 3x caminhada',
    '3x força, 3x cardio leve',
    JSON.stringify({ resistance_days_per_week: 3, cardio_days_per_week: 3 }),
  );
