/**
 * Mobile sign-in with the clinic's canonical credentials (Juliana / 12345678).
 */
import { test, expect } from '@playwright/test';

test.skip(({ isMobile }) => !isMobile, 'Mobile-only');

test('Juliana / 12345678 signs in on a phone viewport', async ({ page }) => {
...
test('API accepts Juliana / 12345678 from a mobile user-agent', async ({ request, baseURL }) => {
